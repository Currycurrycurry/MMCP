...
class DataLoader:
    ...
    def load_data(self, data_type):
        ...
        if (data_type != "train") and (data_type != "test"):
            ...
            print("Start loading {} data".format(data_type))
            if data_type == 'sdk':
                data = pd.read_csv('/root/sdk_data.csv')
            else:
                data = pd.read_csv('/root/rtb.csv')
            max_rows = len(data)
            end_time = time.time()
            print("Loading {} data ({} rows) done.Time cost:{}".format(data_type, max_rows, end_time - start_time))
            return data
