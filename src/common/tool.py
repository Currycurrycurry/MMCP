...
# from config.base_config import B_START, B_DELTA
# from main import B_START, B_LIMIT, B_DELTA

logger = logging.getLogger('tensorflow')
epsilon = sys.float_info.epsilon


def count_pdf(Z, z_size, B_START=0, B_DELTA=1):
    total_num = len(Z)
    z_count = [0 for _ in range(z_size)]
    z_pdf = [0 for _ in range(z_size)]
    for z in Z:
        z_count[int((int(z) - B_START) / B_DELTA - 1)] += 1
    for z in range(z_size):
        z_pdf[z] = z_count[z] / total_num + epsilon
    return z_pdf


def count_cdf(Z, z_size, B_START=0, B_DELTA=1):
    z_cdf = [0 for _ in range(z_size)]
    z_pdf = count_pdf(Z, z_size, B_START=B_START, B_DELTA=B_DELTA)
    prev_z_cdf = 0
    for z in range(z_size):
...
def count_plot_pdf(Z, z_size, label='', B_START=0, B_DELTA=1):
    z_list = [z for z in range(z_size)]
    pdf = count_pdf(Z, z_size, B_START=B_START, B_DELTA=B_DELTA)
...
def plot_mp_result_regression(Z, Z_pred, B_START=0, B_DELTA=1, predict_z_size=301, save_fig=False, show_fig=True, figure_path='', title=''):
    # pdf figure
    figure = plt.figure()
    count_plot_pdf(Z, predict_z_size, 'ground truth', B_START=B_START, B_DELTA=B_DELTA)
    count_plot_pdf(Z_pred, predict_z_size, 'predict', B_START=B_START, B_DELTA=B_DELTA)
